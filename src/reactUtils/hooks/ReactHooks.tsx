// https://github.com/burtonator/polar-bookshelf/blob/master/web/js/hooks/ReactHooks.tsx
...
/**
 * Calls a hook function, but then wraps it in a ref,
 * so that we can always access the same version.
 *
...
/**
 * Use a ref but update the 'current' value each time,
 * so that this could be used with callbacks easier.
 */
...
// Like useState but we also use a ref with the setter function,
// so that it's updated each time as well.
...
// Create a ref for the value and always update it,
// so that inner functions can see the most recent value
